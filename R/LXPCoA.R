...
tab2 <- ggtexttable(dune.pairwise.adonis[,c("pairs","R2","p.value","p.adjusted")], rows = NULL,
  ...
  tab_add_footnote(text="R2:categorical variance/Total variance, indicates the degree of explanation of different
                         classifications for community differences between groups(分类方差/Total方差,表示不同
                         分类对组间群落差异的解释度,其值越大表示分类因子对差异的解释度越大).                   ",
                   face = NULL,
                   size = 7,
                   color = NULL,
                   family = NULL,
 ...
 ggsave("analysis result/PCoA graphics03.png",p_all,width=1000, height =800, dpi=150,units = "px")
